...
**`remindctl show --json upcoming` only returns items with future due dates** — it does NOT return no-due-date items. Use `remindctl show --json all` and filter with jq for no-due-date incomplete items.

...
`chezmoi apply` reloads changed LaunchAgents automatically — the `.chezmoiscripts/run_onchange_after_aa-launch-agents.sh.tmpl` generator renders plists from `dot_config/launchd-yaml/agents.yaml.tmpl` and reloads only what changed. To force-reload an agent manually during testing:
...
- **Deleting an authored skill** — `dot_agents/skills/` is managed directly at `~/.agents/skills/`. Run `chezmoi destroy ~/.agents/skills/<name>` before deleting its source directory; removing source first makes the target unmanaged. Externally installed skills are separate sibling directories and are never erased during authored-skill updates.
...
- **`chezmoi-templates` pre-commit hook renders against the current worktree, not the primary checkout** — it passes `-S "$(git rev-parse --show-toplevel)"`, so it correctly catches templates referencing a `.chezmoidata/*.yaml` key that only exists on the current branch (previously it silently validated against the primary checkout's `main` data and always passed). `.chezmoidata/local.yaml` is gitignored, so worktrees never receive it — templates reading `.secrets` (e.g. `dot_envrc.tmpl`) would otherwise fail this hook in every worktree but the primary checkout. Fixed at the hook, not per-worktree: the entry script layers the tracked `local.yaml.example` (placeholder values, same shape as the real file) on top via `chezmoi execute-template --override-data-file`, copied to a PID-scoped temp path first — `--override-data-file` needs a real `.yaml` extension to autodetect format, and uutils coreutils' `mktemp` seeds its random suffix by wall-clock second, so parallel `--all-files` invocations landing in the same second collided on `mktemp`'s "random" name (`$$`-scoped `cp` avoids both). No workaround needed anymore; if a *new* template starts reading a `.chezmoidata/local.yaml` key, add a placeholder for it to `local.yaml.example` too.

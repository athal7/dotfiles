...
Confluence is a collector input and a publication target. Query CQ for normal context. Use KB when CQ has no answer or projection verification is incomplete.
...
Before any Confluence or Jira write, show the target, complete content, source identity, and linked CQ KU when one exists. Then perform the write.

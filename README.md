...
- **AI tooling**
  ...
  - [Per-org model-class overrides](.chezmoidata/agents.yaml) — a `name`/`class` pair per agentcfg workflow step (kept in sync with [`workflow.yaml.tmpl`](dot_config/agentcfg/workflow.yaml.tmpl)'s own `name`/`class` fields), resolved via `model_class`; a per-org override in `local.yaml`'s `orgs.<org>.model_class` is injected at aoe session launch by [`aoe-model-class`](dot_local/bin/executable_aoe-model-class) (an `on_launch` host hook) into gitignored, project-local `.opencode/opencode.json` and `.omp/config.yml`.
  - [Local model registry](.chezmoidata/models.yaml) — single source of truth for the locally-hosted mlx model (host, port, hf repo, context window); feeds omp's [`models.yml`](dot_omp/private_agent/models.yml.tmpl), the agentcfg-owned opencode mlx provider block, and the `mlx-server` LaunchAgent's `--model`/`--host`/`--port` args.

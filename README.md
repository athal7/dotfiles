...
- **Automation**
  ...
  - [Scheduled AoE sessions](dot_config/launchd-yaml/agents.yaml.tmpl) — one weekday LaunchAgent starts a tmux-backed OMP session that runs production-error triage before knowledge enrichment. Each run has a unique title. The launcher records an opaque per-session correlation token in local state so `/audit` can join scheduled AOE sessions to OMP transcripts. The `aoe-serve` LaunchAgent keeps its local host running after login. Its remote access passphrase is stored in the macOS Keychain.

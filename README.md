...
- **AI tooling**
  ...
  - [Plan-mode skip overlay](dot_omp/private_agent/no-plan.yml) — omp `--config` overlay disabling `plan.defaultOnStartup` (set globally by agentcfg) for one launch; [`aoe-cmd -P`](dot_local/bin/executable_aoe-cmd) applies it when dispatching a scheduled session that must actually write/edit, since plan mode stages edits behind an interactive approval no unattended session can give.

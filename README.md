...
- **AI tooling**
  ...
  - [kb TUI/CLI](.chezmoidata/packages.yaml) — Personal knowledge base engine, interactive Textual TUI dashboard, and CLI (cloned and built directly via `pipx` or `uv tool`).
...
- **Automation**
  - [Git Activity Collector](dot_local/bin/executable_kb-git-activity) — Local Git commit statistics collector that automates daily journal entries using `kb journal append`.

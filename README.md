...
- **AI tooling**
  ...
  - [Local model registry](.chezmoidata/models.yaml) — single source of truth for the locally-hosted mlx model (host, port, hf repo, context window); feeds omp's [`models.yml`](dot_omp/private_agent/models.yml.tmpl), opencode's mlx provider block, and the `mlx-qwen-server` LaunchAgent's `--model`/`--host`/`--port` args.

...
- **`chezmoi-templates` pre-commit hook renders against the current worktree, not the primary checkout** — it passes `-S "$(git rev-parse --show-toplevel)"`, so it correctly catches templates referencing a `.chezmoidata/*.yaml` key that only exists on the current branch (previously it silently validated against the primary checkout's `main` data and always passed). `.chezmoidata/local.yaml` is gitignored, so worktrees never receive it — templates reading `.runlayer`, `.secrets`, or `.orgs` (e.g. `dot_envrc.tmpl`, `dot_config/opencode/opencode.json.tmpl`, `dot_linear-tui/config.json.tmpl`, `dot_omp/private_agent/private_mcp.json.tmpl`) would otherwise fail this hook in every worktree but the primary checkout. Fixed at the hook, not per-worktree: the entry script layers the tracked `local.yaml.example` (placeholder values, same shape as the real file) on top via `chezmoi execute-template --override-data-file`, copied to a PID-scoped temp path first — `--override-data-file` needs a real `.yaml` extension to autodetect format, and uutils coreutils' `mktemp` seeds its random suffix by wall-clock second, so parallel `--all-files` invocations landing in the same second collided on `mktemp`'s "random" name (`$$`-scoped `cp` avoids both). No workaround needed anymore; if a *new* template starts reading a `.chezmoidata/local.yaml` key, add a placeholder for it to `local.yaml.example` too.

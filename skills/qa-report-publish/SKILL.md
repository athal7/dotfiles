...
Self-contained, auto-opened, NEVER pushed. This form embeds QA screenshots as
...
2. **Upsert the report block into the merge request description** between
   ...
   directly with its QA line. Close with a `Could not verify` section. **The
   blank line after each `</summary>` is mandatory** or the inner markdown won't
   render. (See the AC-block layout below.)

...
- **Scratch sessions dodge a broken org model-class override**: a per-org `on_launch` hook rewrites every model class (default/plan/slow/smol) to a local mlx backend for any git-repo-backed session in a repo under that org, to save cost — but that hook's first step is `git rev-parse --show-toplevel`, which fails with no `.git` present. `aoe add --scratch --tool opencode --title <title>` (mutually exclusive with a project path/worktree flags) creates a project-less session under a non-git directory, so the hook no-ops and the session falls back to the global opencode/omp config instead, which already routes default/plan/slow to anthropic. If the local mlx backend is overloaded and a session or dispatched subagent is silently returning empty responses on a repo under that org's model-class override, do the throwaway work in a scratch session (`aoe session start <id>` as usual) rather than a normal repo/worktree session. Use `aoe rm --keep-scratch <id>` when cleaning up if there's output worth keeping.

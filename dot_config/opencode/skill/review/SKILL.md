...
- **Issue context** (requirements, acceptance criteria, project goals) — for correctness and maintainability agents
  - Use the **actual fetched text** from the `linear` skill / `gh issue view` — not the placeholder.
  - If the issue belongs to a Linear project, include the project body and milestone context. This gives reviewers the broader product vision — business goals, related features, existing decisions, and constraints that should inform the review.

...
**Directory discipline.** You and your subagents work only in the current workspace directory. The `build` subagent already refuses to run `git -C <path>` against other repos and returns early rather than operate outside its working directory. When real work (edits, fixes, implementation) is needed in a different directory — a different repo, a sibling worktree, an unrelated project — whether you spot it up front or a subagent returns early for this reason, don't attempt it yourself and don't force a subagent to reach across. Load the `opencode` skill and use `aoe` to start a session scoped to that directory, then `aoe send` the intent of the work (not a step-by-step decomposition) to it. Confirm the session started and the message landed, then move on — this is fire-and-forget, not a session to babysit synchronously.

...
1. Call `searchMeetings` with the window. Filter to meetings where `has_summary` or `has_transcript` is true.
...
4. **Stop when you have what was asked for.** Do not over-fetch. One `searchMeetings` + targeted `get_meeting_assets` calls is the typical pattern.

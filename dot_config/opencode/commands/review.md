...
- Submit — assemble the unified review report (reviewer findings + qa evidence, by AC) as your local worktable, then deliver by ownership (`review-publish` skill) after approval — inline comments only for someone else's PR (no hosting), description block + hosting for your own
...
Lead ASSEMBLES the unified review report in BOTH forms: interleave the reviewer's AC-grouped findings with qa's per-AC evidence, one section per acceptance criterion, into `review-report.html` (local-only — embeds the rendered diffs + screenshots + running-app links) AND `review-report.md` (deep-links the diffs, relative-ref screenshots) in qa's session dir (so the relative screenshot refs resolve). When QA did NOT run, lead still creates the `qa-<ts>` dir and writes both forms with verdict `n/a` (the diff/findings half is always present). These forms are the reviewer's **worktable/record** — always generated locally. Then load the `review-publish` skill and follow it — open the `.html` locally → approval gate → deliver by ownership:

- **Someone else's merge request** (the usual `/review` target): submit ONE review — inline line-anchored comments drafted from the reviewer's findings, with an EMPTY top-level body (no verdict badge, no finding counts, no per-AC outline, no report link). The inline comments are the whole deliverable; **host nothing** (do not push the `.md`, do not link a report). **Strip the internal `[build]`/`[human]`/`[plan]` routing tags from the author-facing inline comment bodies** — write self-contained, actionable prose; the classification stays internal (it organizes the local report). `event` is REQUEST_CHANGES with any blocker, COMMENT for nits/questions only, APPROVE (empty body, no inline comments) when clean — and APPROVE ALWAYS goes through the explicit human approval gate; never auto-approve.

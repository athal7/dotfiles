...
Create an OpenSpec proposal to persist the plan: dispatch the `build` subagent (`task` tool, `subagent_type: build`) with `openspec-propose` to create proposal + design + tasks. The proposal is the plan artifact — it captures what changes, why, and the task breakdown. This step is mandatory, not conditional on change size. If the plan's Domain model section proposes new or changed terms, materialize them as a delta spec at `changes/<name>/specs/domain-model/spec.md` — one Requirement per term or bounded context (heading `Term: <X>` / `Bounded context: <X>`) with a `#### Scenario:` expressing its canonical meaning and boundary, valid under `openspec validate` — so they persist into the durable `openspec/specs/domain-model/` via the existing reviewed non-lossy merge at Ship (implement.md's Ship phase already folds all `changes/<name>/specs/` deltas; no Ship-phase change is needed).

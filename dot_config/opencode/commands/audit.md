...
**DCP firing signal + segmented R3 grade + re-derived floor.** These snippets are snip-shell-safe (one `jq` pipeline or one `sqlite3 -readonly` per command, file/value inputs via command substitution, no heredoc, no bare `VAR=`, no shell read-loop, inline literal paths, `\$` escaped). They surface (1) the firing aggregate, (2) the DB compress lower bound, (3) the segmented pre/post reduction, (4) the candidate floor under the work-intensity guard, and (5) the committed-floor R3 grade. The two *refusal guards* (boundary-straddle refusal, ratchet refusal) come FIRST — they constrain the grades that follow.

...
3. **Pre-dispatch dedup check.** Before dispatching, run `cq query "<error.exception.type>: <error.exception.message>"` to check whether this exact error/exception type or error-group signature was already triaged or fixed in a prior knowledge unit. If `cq query` returns a matching unit whose disposition indicates the issue was already fixed, skip dispatching this group — the fix is already recorded. If the prior unit shows the issue was triaged but not fixed, or if there's evidence of recurrence (e.g. the same error reappeared after the prior fix window), dispatch anyway and note the recurrence in the prompt.
...
5. **Report** — error groups found (with counts), sessions dispatched (worktree paths + branch names), and the top unmapped noise service/group noted for awareness.

...
def busy_intervals(events, tz):
    ...
    for e in events:
        if e.get("all_day"):
            continue
        if (e.get("notes") or "").strip() == TAG:
            continue
        if e.get("availability") == "free":
            continue
        if "lunch" in e.get("title", "").lower():
            continue
...
def process_day(day, cal, tz):
    ...
    has_lunch_meeting = any(
        ...
        and e.get("notes", "").strip() != TAG
        for e in events
        if e.get("availability") != "free" and not e.get("all_day")
    )
